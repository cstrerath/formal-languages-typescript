...
class RecursiveSet<T extends Value> implements Structural, Iterable<T> {
    ...
    /**
     * Checks if this set is a subset of the other set (A ⊆ B).
     * Complexity: O(N) where N is the size of 'this'.
     */
    isSubset(other: RecursiveSet<T>): boolean {
        // Optimization 1: A subset cannot be larger than the superset
        if (this.size > other.size) return false;

        // Optimization 2: Identity check
        if (this === other) return true;

        // Check if every element of 'this' exists in 'other'
        // Since other.has() is O(1), this loop runs in O(this.size).
    ...
    /**
     * Checks if this set is a superset of the other set (A ⊇ B).
     */
