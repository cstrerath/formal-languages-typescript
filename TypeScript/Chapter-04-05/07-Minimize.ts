...
import { DFA, DFAState, State, Char, TransRelDet, key, TransitionKey } from "./01-NFA-2-DFA";
...
function allSeparable(Q: RecursiveSet<DFAState>, A: RecursiveSet<DFAState>, Sigma: RecursiveSet<Char>, delta: TransRelDet): RecursiveSet<Pair> {
...
function reachable(q0: DFAState, Sigma: RecursiveSet<Char>, delta: TransRelDet): RecursiveSet<DFAState> {
...
export function minimize(F: DFA): MinDFA {
  ...
  for (const q of Q) { if (A.has(q)) reachableA.add(q); }
